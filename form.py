...
def form(browser, config, keys, domain):
    ...
    while True:
        try:
            ...
            username.clear()
            username.send_keys(config.get('pass', 'formLog'))
            password = browser.find_element_by_name('passwd')
            password.clear()
            password.send_keys(config.get('pass', 'formPass'))
    ...
    print('Generowanie klucza rsForm...')
    browser.get('https://www.rsjoomla.com/my-memberships/add-new-license/143789.html?tmpl=component')
    while True:
        try:
